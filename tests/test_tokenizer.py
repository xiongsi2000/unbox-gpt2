    ...
    def decorator(f):
        def wrapper(*args, **kwargs):
            if not RESOURCE_AVAILABLE:
                # Skip memory limiting if dependencies are not available
...
def bytes_to_str_utf8(vocab):
    return {key: value.decode('utf-8') for key, value in vocab.items()}


def bytes_to_str_latin1(vocab):
    return {key: value.decode('latin1') for key, value in vocab.items()}
...
def test_empty_matches_tiktoken():
    if not TIKTOKEN_AVAILABLE:
        return  # Skip test if tiktoken is not available
...
def test_encode_iterable_memory_usage():
    if not PYTEST_AVAILABLE or not RESOURCE_AVAILABLE:
        return  # Skip test if dependencies are not available
